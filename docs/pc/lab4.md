...
Un vector este o colectie de elemente de acelasi tip de date.

Elementele unui vector sunt stocate in memorie in mod continuu.
...
```
dimensiunea_vectorului = numar_elemente * dimensiune_element
```

:::

:::warning Atentie
Indexul primului element este `0`, iar ultimul element are indexul `dimensiune_vectorului - 1`.
:::
...
:::info Observatie
Linia de cod declară un vector de intregi cu 5 elemente, dar nu initializeaza elementele. 

Puteam să avem și alte tipuri de date, cum ar fi `double`, `char`, `string`, etc.
:::

:::warning Atentie
Dimensiunea unui vector trebuie sa fie o valoare constanta, cunoscuta la compilare.
:::
...
:::warning Atentie
Accesarea unui element din afara limitelor vectorului duce la comportament nedefinit. 
:::
...
```
dimensiunea_matricei = numar_linii * numar_coloane * dimensiune_element 
```
...
:::info Observatie
Primul ciclu `for` parcurge liniile matricei, iar al doilea ciclu `for` parcurge coloanele matricei.
:::
